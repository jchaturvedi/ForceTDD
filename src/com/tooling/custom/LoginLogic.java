...
public class LoginLogic {
	...
	public static com.sforce.soap.tooling.SoapConnection toolingConnection = null ;
	String loginStatus ;
	com.sforce.soap.partner.LoginResult result = null;
	private  WordprocessingMLPackage wordMLPackage;
	
	public  String initMethod(String userName, String password, Boolean isProd){
		...
		try {
			...
			System.out.println("connector.END_POINT :" +Connector.END_POINT);
//			result = connection.login(userName, password);
			System.out.println("User logged in");
			// display some current settings
			System.out.println("Auth EndPoint: "+partnerConfig.getAuthEndpoint());
			System.out.println("Service EndPoint: "+partnerConfig.getServiceEndpoint());
			System.out.println("Username: "+partnerConfig.getUsername());
			System.out.println("SessionId: "+partnerConfig.getSessionId());
			...
			System.out.println("Getting tooling connection");
			
			toolingConnection = com.sforce.soap.tooling.Connector.newConnection(toolingConfig);
			System.out.println("Got tooling connection");
			
		} catch (ConnectionException e) {
			System.out.println("LoginLogic> Exception :" + e);
			ApiFault apiFault = (ApiFault) e;
			e.printStackTrace();
			String loginStatus =  apiFault.getExceptionMessage();
			System.out.println("LoginLogic> loginStatus :" + loginStatus);
			return loginStatus;
		}
		...
		if( toolingConnection.getSessionHeader().getSessionId() != null){
			loginStatus = DDConstants.SUCCESS;
	...
	public void fetchApexClassMethod(String s){
		System.out.println(" IN MAIN METHOD" + s);
		QueryResult classList = null;
		//SObject[] apexClassList = null;
		try {
			classList = toolingConnection.query("SELECT Id,  Name, Status, LastModifiedDate, FullName FROM ApexClass WHERE Id IN " + s);
			//apexClassList = toolingConnection.retrieve("Id, Name, FullName", "ApexClass", s);
		} catch (ConnectionException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		System.out.println("fetchApexClassMethod QueryResult SIZE:--- "+ classList);
		...
		try {
			//File file=new File("\\HerokuProject\\WebContent\\Quality Initiative_TDD.docx");
